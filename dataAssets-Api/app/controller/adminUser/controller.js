 ...
 * @apiGroup adminUser
 * @apiExample 请求示例:
 * curl -H "Content-Type: application/json" -X POST -d '{ "username": username ,"password": password }' 'http://localhost:3003/adminUser/login'
...
export async function  userLogin(ctx) {
    ...
    if(!admin){
        ctx.body = NET.paramUserError("无当前用户");
        return;
    }
    const isMatch = await admin.validatePassword(param.password);
    if (!isMatch) {
        ctx.body = NET.paramPwdError("密码不正确");
        return;
    }
    //生成token
 ...
 * @apiGroup adminUser
 * @apiExample 请求示例:
 * curl -H "Content-Type: application/json" -X POST -d '{ }' 'http://localhost:3003/adminUser/loginOut'
 *
 *
 ...
 * @apiGroup adminUser
 * @apiExample 请求示例:
 * curl -H "Content-Type: application/json" -X POST -d '{ "username": username ,"password": password ,"sex":sex,"role_id":role_id,"email":email,"mobile":mobile}' 'http://localhost:3003/adminUser/login'
 * @apiParam {String} username                   用户名.
 * @apiParam {String} password                   密码.
