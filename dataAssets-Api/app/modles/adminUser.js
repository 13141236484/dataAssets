...
function generateToken() {
    const adminUser = this;
    // ,{expiresIn: 60*60*24}
    return jwt.sign({ id: adminUser.id }, config.token);
}
...
async  function getUserByEmail(email){
    return await AdminUser.findAll({where:{email:email},attributes:['id']});
}
async  function getUserByMobile(phone){
    return await AdminUser.findAll({where:{mobile:phone},attributes:['id']});
}
async function getUserByUserName(name){
    return await AdminUser.findAll({where:{username:name},attributes:['id']});
}
//查询单条用户名字
async function getAdminUserName(userId){
    return await AdminUser.findOne({where:{id:userId},attributes:['id','username']});
}
